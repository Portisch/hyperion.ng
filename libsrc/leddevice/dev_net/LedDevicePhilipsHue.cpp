...
const int SSDP_TIMEOUT = 30000; // timout in ms
...
bool LedDevicePhilipsHueBridge::discoverDevice()
{
	...
	address = discover.getFirstService(STY_WEBSERVER, "wifi_bulb", SSDP_TIMEOUT);
	address = discover.getFirstService(STY_WEBSERVER, "upnp:rootdevice", SSDP_TIMEOUT);

	// Discover Philips Hue Bridge
	address = discover.getFirstService(STY_WEBSERVER, SSDP_ID, SSDP_TIMEOUT);

	//discover.searchForService("aaa");
...
bool LedDevicePhilipsHue::initLights()
{
	bool isInitOK = false;
...
{
	Debug(_log, "LedDevicePhilipsHue::open()");
	int retval = -1;
	_deviceReady = false;

	// Open LedDevice based on configuration
	...
	Debug(_log, "LedDevicePhilipsHue::open() [%d]", retval);
	return retval;
}

void LedDevicePhilipsHue::restoreOriginalState()
{
	Debug(_log, "LedDevicePhilipsHue::restoreOriginalState()");
	if(!_lightIds.empty())
	{
...
{
	Debug(_log, "LedDevicePhilipsHue::stop()");
...
{
	Debug(_log, "LedDevicePhilipsHue::switchOn(), enabled [%d], _deviceReady [%d]", this->enabled(), _deviceReady);
	bool rc = false;
	if (  LedDevice::switchOn() )
	{
	...
	Debug(_log, "LedDevicePhilipsHue::switchOn()- rc [%d], enabled [%d], _deviceReady [%d]", rc, this->enabled(), _deviceReady);
	return rc;
}

bool LedDevicePhilipsHue::switchOff()
{
	Debug(_log, "LedDevicePhilipsHue::switchOff(), enabled [%d], _deviceReady [%d]", this->enabled(), _deviceReady);
	...
	Debug(_log, "LedDevicePhilipsHue::switchOff()- rc [%d], enabled [%d], _deviceReady [%d]", rc, this->enabled(), _deviceReady);
	return rc;
}
